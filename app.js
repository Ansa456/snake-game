...
function move(){
    if(
        ...
        (currentSnake[0] - width < 0 && direction === -width) || // if snake hit up
        squares[currentSnake[0] + direction].classList.contains('snake')
    )
        return clearInterval(timerId);

    const tail = currentSnake.pop();// currentSnake = [2,1]
   
    

    squares[tail].classList.remove('snake');

    currentSnake.unshift(currentSnake[0] + direction); // currentSnake = [3,2,1]
